...
description: "Selective human review changes which model errors we observe, and therefore the correction an estimate needs. Routing and correction have to be designed together."
date: 2026-09-21
...
Using model labels to estimate a population quantity requires accounting for both the model's errors and how the human labels are collected. Selective review changes which errors we observe, and therefore the correction an estimate needs. This post examines how routing and correction should be designed together.

A short example shows why model error alone is not the right quantity to watch. Two classifiers each make 120 errors on 1,000 items, so both are 88% accurate. One makes 60 false positives and 60 false negatives; the other makes 90 and 30. Their prevalence estimates are off by zero and by +6 percentage points. Classification error depends on the *sum* of the two error masses; prevalence error depends on their *difference*.

I organize the discussion around four stages: **generate** candidate labels, **route** items to a judge or to a person, **correct** estimates using the human labels, and **audit** the reference labels themselves. The first stage gets one paragraph, since its lesson is a premise for the rest. The four-stage framing and the argument about routing and correction are my own reading of the literature; claims about individual papers are attributed to them, and places where I go beyond a paper are marked.
...
## A premise: validation is task-specific

Annotation quality depends on the model, the data distribution, and the label definition together, so a validation result on one task carries little over to another. Pangakis et al. [1] replicated 27 social-science annotation tasks and found a median accuracy of 0.85 alongside precision or recall below 0.5 on nine of them. Prompting closes only part of a gap between the model's prior and a task's definition: in a nine-model toxicity study, Casanova et al. [2] found that prompting rescued 34.8% of zero-shot errors overall and 20.8% of those made with confidence above 0.9, and that supplying the matching definition raised mean accuracy across the nine models from 80.3% to 82.0%. Everything below assumes the model has been validated on the task at hand, and takes for granted that some of its errors will persist, especially the confident ones.
