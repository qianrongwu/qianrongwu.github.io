...
where $\sigma_R$ is the group reward standard deviation and $\delta>0$ guards against division by zero. In this outcome-supervised version, the same $A_i$ is used for every token in response $i$. The original objective retains token ratios and clipping, averages tokens within each response, and includes a Kullback–Leibler (KL) divergence penalty relative to a reference policy. [DeepSeekMath, §4.1](https://arxiv.org/html/2402.03300v3#S4.SS1)
...
Early observations predict later performance in their experiments. The authors also explicitly discuss settings with different entropy dynamics. The fitted endpoint $b-a$ is therefore a conditional extrapolation, not a universal ceiling on what RL can learn. [Cui et al., §§2.4–2.6](https://arxiv.org/html/2505.22617v1#S2.SS4)
...
This calculation describes an idealized multiplicative update at a fixed prefix. Cui et al.'s tabular analysis distinguishes vanilla policy gradient, involving $\operatorname{Cov}(\log\pi,\pi A)$, from natural policy gradient, involving $\operatorname{Cov}(\log\pi,A)$. The observed tendency toward positive covariance does not imply monotonic entropy decline for every optimizer and training recipe. [Cui et al., §3](https://arxiv.org/html/2505.22617v1#S3)

The same work introduces Clip-Cov and KL-Cov, which target selected tokens' contributions through gradient suppression or KL regularization. These methods explicitly address entropy dynamics. [Cui et al., §4](https://arxiv.org/html/2505.22617v1#S4)
...
DAPO also removes the reference-policy KL term. Its Clip-Higher ablation shows higher policy entropy and more diverse samples. [Yu et al., §§2.3–3.4](https://arxiv.org/html/2503.14476v2#S3)
...
The paper reports 50% on AIME 2024 with Qwen2.5-32B, averaging accuracy over 32 repetitions. That is avg@32, not pass@32. Its reported reduction in update steps should not be read as an equal reduction in total compute. [DAPO, §4](https://arxiv.org/html/2503.14476v2#S4)
...
Length normalization controls the scale of the weight, but $s_i$ is a surrogate weight: the exact change-of-measure ratio is $\rho_i$. The paper uses much tighter clipping widths for this different quantity. [Zheng et al., §§4–5](https://arxiv.org/html/2507.18071v2#S4)
...
The paper reports improved training stability and efficiency, including mixture-of-experts (MoE) training without Routing Replay, which reuses the old policy's expert selections during updates. The reported roughly 10% change in activated experts concerns a particular Qwen3-30B-A3B setup, not a universal rate for every MoE token. [GSPO, §5.3](https://arxiv.org/html/2507.18071v2#S5.SS3)
